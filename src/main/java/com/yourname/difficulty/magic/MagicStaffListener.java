...
public class MagicStaffListener implements Listener {
    ...
    /** Water blocks placed/created by the Water Staff — tracked so the Earth Staff's
     *  Sandstorm right-click can identify "magic water" specifically (not any water). */
    private final Set<Location>                      magicWaterBlocks   = new HashSet<>();
    /** Per-player Lv99 permanent Air flight state: whether currently flying. */
    private final Set<UUID>                          permaFlightActive  = new HashSet<>();
    /** Per-player Lv99 permanent Air flight drain task. */
    private final Map<UUID, BukkitTask>              permaFlightTasks   = new HashMap<>();

    private       SandstormManager                   sandstormManager   = null;
    /** Optional WeatherStormManager — controls Water Lv99 Downpour rain + Earth Lv99 Sand Rain. */
    private       WeatherStormManager                weatherStormManager = null;
    ...
    /** Tracks players currently channeling the earth sand-rain spell. */
    private final Set<UUID>                          channelingEarth       = new HashSet<>();
    /** Metadata key set by the damage listener when a channeling player is hit — checked each channel tick. */
    private static final String META_CHANNEL_HIT = "magic_channel_hit";

    /** Water Runes required to complete the Downpour channel ritual. */
    private static final int  WATER_DOWNPOUR_RUNE_COST   = 64;
    /** How long the Downpour buff/storm stays active once triggered (8 minutes). */
    private static final long WATER_DOWNPOUR_DURATION_MS = 8L * 60L * 1000L;
    /** Player UUID → epoch-ms timestamp when their active Downpour buff expires. */
    private final Map<UUID, Long> downpourActiveUntil = new HashMap<>();
    /** Player UUID → repeating upkeep task that drains Water Runes once Downpour has been active 20s+. */
    private final Map<UUID, BukkitTask> downpourUpkeepTasks = new HashMap<>();
    /** Downpour upkeep: grace period before rune drain kicks in. */
    private static final long DOWNPOUR_UPKEEP_GRACE_MS = 20_000L;
    /** Downpour upkeep: Water Runes drained per second once the grace period elapses. */
    private static final int  DOWNPOUR_UPKEEP_RUNES_PER_SEC = 5;
    /** Earth Sand Rain channel duration in seconds (shorter than Water's 10s — it's an upgrade, not a fresh ritual). */
    private static final int  EARTH_SANDRAIN_CHANNEL_SECONDS = 6;
    /** Sand Rain upkeep cost — split 50/50 between Water and Earth runes, plus one Sand block for damage. */
    private static final int  SANDRAIN_WATER_RUNE_COST = 5;
    private static final int  SANDRAIN_EARTH_RUNE_COST = 5;
    ...
    /** Wires in the WeatherStormManager for Water Lv99 Downpour rain + Earth Lv99 Sand Rain. */
    public void setWeatherStormManager(WeatherStormManager wsm) { this.weatherStormManager = wsm; }
    ...
    /** Wires in the ElementalProcManager for real dice-roll passive procs on basic hits. */
    public void setElementalProcManager(ElementalProcManager epm) { this.elementalProcManager = epm; }
    ...
    public void setPartyManagerRef(com.yourname.difficulty.party.PartyManager pm) {
        this.partyManagerRef = pm;
        if (sandstormManager != null) sandstormManager.setPartyManager(pm);
    }

    /**
     * Returns the remaining active-Downpour time (ms) for this player, or 0 if
     * they have no active Downpour buff. Used to cap the Earth Staff's
     * Sandstorm duration — the storm can never outlast the Water spell that
     * empowers it.
     */
    public long getRemainingDownpourMs(Player player) {
        if (player == null) return 0L;
        Long expiry = downpourActiveUntil.get(player.getUniqueId());
        if (expiry == null) return 0L;
        long remaining = expiry - System.currentTimeMillis();
        if (remaining <= 0) {
            downpourActiveUntil.remove(player.getUniqueId());
            return 0L;
        }
        return remaining;
    }
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        // ── Fix: PlayerInteractEvent fires once per hand (MAIN + OFF_HAND) for a
        // single physical click. Without this guard, every right/left click was
        // processed TWICE, which effectively let players bypass cooldowns / burn
        // through two casts (and two rune costs) per click. Only ever process
        // the event for the main hand.
        ...
        if (element == MagicElement.WATER) {
            if (isRightClick) {
                // If Downpour is already active, right-clicking Water toggles it
                // OFF early instead of starting a new channel.
                if (downpourActiveUntil.containsKey(player.getUniqueId())) {
                    downpourActiveUntil.remove(player.getUniqueId());
                    if (weatherStormManager != null) weatherStormManager.stopDownpour(player.getWorld());
                    player.sendMessage("§b💧 §7Downpour §cdeactivated§7 manually.");
                    player.sendActionBar("§b💧 §7Downpour deactivated!");
                    player.playSound(player.getLocation(), Sound.ITEM_BUCKET_EMPTY, 0.8f, 1.2f);
                    return;
                }
                // Right-Click (Lv99 Capstone): Channeling Downpour — no fallback,
                // clear message if requirements aren't met.
                if (magicLevel < 99) {
                    player.sendActionBar("§c✗ §7Downpour is a Capstone spell requiring §eMagic Level 99§7!");
                    return;
                }
                if (!itemFactory.hasWaterBook(player, getBagContents(player))) {
                    player.sendActionBar("§c✗ §7Downpour requires carrying §bThe Water Book§7!");
                    return;
                }
                ...
                rollRuneBreak(player, MagicElement.WATER);
        ...
        if (element == MagicElement.EARTH) {
            if (isRightClick) {
                // ── Right-Click (Lv99 Capstone): SAND RAIN channeling ritual ──
                // Requires Magic Lv99 + Earth Book. Only actually triggers the
                // damaging Sand Rain effect while a Water Staff Downpour is
                // currently active somewhere in this world; otherwise tells the
                // player exactly what's missing. Mirrors Water's Downpour
                // channel (cancel on move/hit) but shorter — it's an upgrade,
                // not a fresh ritual.
                if (magicLevel < 99) {
                    player.sendActionBar("§c✗ §7Sand Rain is a Capstone spell requiring §eMagic Level 99§7!");
                    return;
                }
                if (!itemFactory.hasEarthBook(player, getBagContents(player))) {
                    player.sendActionBar("§c✗ §7Sand Rain requires carrying §2The Earth Book§7!");
                    return;
                }
                if (weatherStormManager == null) {
                    player.sendActionBar("§c✗ §7Weather system unavailable — contact an admin.");
                    return;
                }
                if (!weatherStormManager.isWeatherEventActive(player.getWorld())) {
                    player.sendActionBar("§c✗ §7Sand Rain needs an active §bDownpour§7! §8(Cast Water Lv99 right-click first)");
                    return;
                }
                if (channelingEarth.contains(player.getUniqueId())) {
                    player.sendActionBar("§c✗ §7You are already channeling!");
                    return;
                }
                startEarthSandRainChannel(player, magicLevel);
                return;
    ...
    /**
     * Public wrapper used by the Support Staff's left-click "Infused" flurry
     * (CastingEngine.executeSupportBasicAttack) to fire each active elemental
     * staff's REAL basic-tier spell instead of a simplified combined snowball.
     * Dispatches to the same basic cast methods used by direct staff casts —
     * no rune consumption here (the caller already consumed the element's rune).
     */
    public void castElementForSupport(Player player, MagicElement element, int magicLevel) {
        switch (element) {
            case FIRE  -> castFire(player, magicLevel,
                    magicLevel >= 10 && itemFactory.hasSpellComboBook(player, getBagContents(player)));
            case WATER -> castWater(player, magicLevel, Action.LEFT_CLICK_AIR);
            case EARTH -> castEarthBasicBolt(player, magicLevel);
    ...
    private void castLightning(Player player, int magicLevel) {
        ...
        // ── Downpour / Sand Rain weather modifier ─────────────────────────────
        // Water empowers lightning: a pure Downpour (no sand) buffs lightning
        // damage +15%. Once Sand Rain is active, the sand "grounds" the bolt
        // and nerfs it -15% instead. Only applies while a weather event is
        // actually active in the caster's world.
        String weatherTag = "";
        if (weatherStormManager != null && weatherStormManager.isWeatherEventActive(player.getWorld())) {
            if (weatherStormManager.isSandRainActive(player.getWorld())) {
                baseDmg *= 0.85;
                weatherTag = " §8(§2Sand Rain §8-15%§8)";
            } else {
                baseDmg *= 1.15;
                weatherTag = " §8(§bDownpour §8+15%§8)";
            }
        }
        ...
        for (Entity e : player.getWorld().getNearbyEntities(strikeAt, 3, 4, 3)) {
            ...
            // ── Wet/Damp lightning bonus ──────────────────────────────────────
            // Water conducts! A Wet/Damp target takes extra lightning damage.
            // If the caster carries the Water Support Page, the bonus is
            // amplified further (water magic empowering water magic).
            if (isWet(le) || isDamp(le)) {
                double wetBonus = 1.25;
                if (itemFactory.hasSupportPage(player, ItemFactory.SUPPORT_PAGE_HEALING_KEY)) {
                    // (Water's associated support page — amplifies the wet-lightning synergy)
                    wetBonus = 1.45;
                }
                finalDmg *= wetBonus;
                le.getWorld().spawnParticle(Particle.SPLASH, le.getLocation().add(0, 1, 0), 20, 0.4, 0.4, 0.4, 0.1);
                if (le instanceof Player wp) wp.sendActionBar("§b⚡ §7Wet/Damp — lightning surged through the water!");
            }
    ...
    // ══════════════════════════════════════════════════════════════════════════
    //  PLAYER QUIT CLEANUP — prevents unbounded memory growth over long uptime
    // ══════════════════════════════════════════════════════════════════════════

    /**
     * Cleans up ALL per-player state tracked by this listener when a player
     * disconnects. Without this, every player who ever cast a spell would
     * leave permanent entries behind in the various tracking maps/sets below,
     * growing without bound over a long-running server's lifetime (a slow
     * memory leak). Cancels any active repeating tasks for the player first,
     * then purges every UUID-keyed collection.
     */
    ...
    public void onPlayerQuit(PlayerQuitEvent event) {
        ...
        // Cancel any active repeating tasks tied to this player first
        cancelFallenTask(uuid);
        cancelHoverTask(uuid);
        cancelPermaFlight(uuid);
        BukkitTask lightningBurn = lightningBurnTasks.remove(uuid);
        if (lightningBurn != null) lightningBurn.cancel();
        BukkitTask downpourUpkeep = downpourUpkeepTasks.remove(uuid);
        if (downpourUpkeep != null) downpourUpkeep.cancel();

        // Purge every per-player tracking collection
        cooldowns.remove(uuid);
        guidedPlayers.remove(uuid);
        downpourActiveUntil.remove(uuid);
        channelingWater.remove(uuid);
        channelingEarth.remove(uuid);
        permaFlightActive.remove(uuid);

        // Projectile-related maps are keyed by projectile UUID (not player UUID)
        // and are already cleaned up per-projectile in onProjectileHit, but any
        // shot still in flight when the shooter quits is harmless (values are
        // just a UUID/enum/int, and stale entries are removed the moment that
        // specific projectile is resolved). No further action needed there.
    }
    ...
    private void startWaterDownpourChannel(Player player, int magicLevel) {
        ...
        // ── Escape mechanic: channeling Downpour extinguishes the caster's own
        // Fire-proc Burn status immediately (see ElementalProcManager.applyBurn).
        if (elementalProcManager != null && elementalProcManager.isBurning(player)) {
            elementalProcManager.extinguishBurn(player);
        }
        ...
        new org.bukkit.scheduler.BukkitRunnable() {
            ...
            public void run() {
                ...
                // ── Externally cancelled (e.g. took damage) ───────────────────
                if (!channelingWater.contains(uuid)) {
                    cancel();
                    return;
                }
                ...
                if (seconds >= 10) {
                    ...
                    // ── 64 Water Rune ritual cost ──────────────────────────────
                    int haveRunes = countRunes(player, MagicElement.WATER);
                    if (haveRunes < WATER_DOWNPOUR_RUNE_COST) {
                        // Fallback: not enough runes — cancel gracefully, no partial
                        // consumption, and tell the player exactly what they need.
                        player.sendActionBar("§c✗ §7Downpour fizzled! §8Need §b"
                                + WATER_DOWNPOUR_RUNE_COST + " §7Water Runes §8(have " + haveRunes + ")§7.");
                        player.sendMessage("§c✗ §7The Downpour ritual requires §b"
                                + WATER_DOWNPOUR_RUNE_COST + " Water Runes§7 — you only have §e" + haveRunes + "§7.");
                        player.playSound(player.getLocation(), Sound.ITEM_BUCKET_EMPTY, 0.8f, 0.6f);
                        return;
                    }
                    for (int i = 0; i < WATER_DOWNPOUR_RUNE_COST; i++) consumeRune(player, MagicElement.WATER);

                    // ── Activate 8-minute Downpour window ──────────────────────
                    downpourActiveUntil.put(uuid, System.currentTimeMillis() + WATER_DOWNPOUR_DURATION_MS);

                    awardMagicXp(player, MAGIC_XP_CAST);
                    executeDownpour(player, magicLevel);
                    if (weatherStormManager != null) {
                        weatherStormManager.startDownpourRain(player);
                    }
                    player.sendMessage("§b💧 §l§bDOWNPOUR! §7The ritual is complete — active for §e8 minutes§7!");
                    player.sendTitle("§b§l💧 DOWNPOUR", "§7Active for 8 minutes!", 5, 60, 15);

                    // ── Start the rune-upkeep task: after 20s active, drains 5
                    // Water Runes/sec (checking inventory + Magic Bag). If runes
                    // run dry mid-upkeep, force-deactivate the Downpour early.
                    startDownpourUpkeep(player);
                    return;
    ...
    // ══════════════════════════════════════════════════════════════════════════
    //  DOWNPOUR RUNE UPKEEP — 20s grace period, then 5 Water Runes/sec
    // ══════════════════════════════════════════════════════════════════════════

    /**
     * Starts the Downpour upkeep task for a player whose 8-minute Downpour
     * buff has just activated. Waits {@link #DOWNPOUR_UPKEEP_GRACE_MS} (20s),
     * then drains {@link #DOWNPOUR_UPKEEP_RUNES_PER_SEC} Water Runes every
     * second (checking both main inventory AND Magic Bag via
     * {@link #consumeRuneDual}). If runes run dry mid-upkeep, force-deactivates
     * the Downpour early with a clear message. Cancelled automatically if the
     * player manually deactivates (right-click toggle-off already clears
     * downpourActiveUntil — this task detects that and stops itself), logs out,
     * or the Downpour's natural 8-minute duration expires.
     */
    private void startDownpourUpkeep(Player player) {
        UUID uuid = player.getUniqueId();

        // Cancel any pre-existing upkeep task for this player first
        BukkitTask old = downpourUpkeepTasks.remove(uuid);
        if (old != null) old.cancel();

        BukkitTask task = plugin.getServer().getScheduler().runTaskTimer(plugin, () -> {
            // Stop if the player logged out or the Downpour buff is no longer tracked
            // (either it expired naturally or was manually/force deactivated).
            if (!player.isOnline() || !downpourActiveUntil.containsKey(uuid)) {
                BukkitTask self = downpourUpkeepTasks.remove(uuid);
                if (self != null) self.cancel();
                return;
            }

            if (countRunesDual(player, MagicElement.WATER) < DOWNPOUR_UPKEEP_RUNES_PER_SEC) {
                // Not enough runes to sustain the storm — force-deactivate early.
                downpourActiveUntil.remove(uuid);
                if (weatherStormManager != null) weatherStormManager.stopDownpour(player.getWorld());
                player.sendMessage("§c✗ §7Downpour §cfizzled out§7 — ran out of §bWater Runes§7!");
                player.sendActionBar("§c✗ §7Downpour ended — out of Water Runes!");
                player.playSound(player.getLocation(), Sound.ITEM_BUCKET_EMPTY, 0.8f, 0.6f);
                BukkitTask self = downpourUpkeepTasks.remove(uuid);
                if (self != null) self.cancel();
                return;
            ...
            for (int i = 0; i < DOWNPOUR_UPKEEP_RUNES_PER_SEC; i++) consumeRuneDual(player, MagicElement.WATER);
        }, DOWNPOUR_UPKEEP_GRACE_MS / 50L, 20L); // grace period delay (ticks), then every 1s

        downpourUpkeepTasks.put(uuid, task);
    }

    // ══════════════════════════════════════════════════════════════════════════
    //  EARTH SAND RAIN — channeling ritual (mirrors Water's Downpour channel)
    // ══════════════════════════════════════════════════════════════════════════

    /**
     * Channels the Earth Staff's Sand Rain upgrade over
     * {@link #EARTH_SANDRAIN_CHANNEL_SECONDS} seconds — cancels on movement or
     * getting hit (see onChannelHitCancel, which also checks channelingEarth).
     * On successful completion: consumes the 50/50 Water+Earth rune cost plus
     * one Sand block (checking inventory + Magic Bag for both rune types),
     * then activates Sand Rain via WeatherStormManager.
     */
    private void startEarthSandRainChannel(Player player, int magicLevel) {
        UUID uuid = player.getUniqueId();
        channelingEarth.add(uuid);
        Location startLoc = player.getLocation().clone();
        ...
        new org.bukkit.scheduler.BukkitRunnable() {
            ...
            public void run() {
                ...
                // ── Externally cancelled (e.g. took damage) ───────────────────
                if (!channelingEarth.contains(uuid)) {
                    cancel();
                    return;
                }
                if (player.getLocation().distanceSquared(startLoc) > 4.0) {
                    player.sendActionBar("§c✗ §7Sand Rain channel cancelled — you moved too much!");
                    cancel();
                    channelingEarth.remove(uuid);
                    return;
                }
                ItemStack hand = player.getInventory().getItemInMainHand();
                if (itemFactory.getStaffElement(hand) != MagicElement.EARTH) {
                    player.sendActionBar("§c✗ §7Sand Rain channel cancelled — item switched!");
                    cancel();
                    channelingEarth.remove(uuid);
                    return;
                }
                // Downpour must still be active to channel into Sand Rain
                if (weatherStormManager == null || !weatherStormManager.isWeatherEventActive(player.getWorld())) {
                    player.sendActionBar("§c✗ §7Sand Rain channel cancelled — Downpour ended!");
                    cancel();
                    channelingEarth.remove(uuid);
                    return;
                }
                ...
                if (seconds >= EARTH_SANDRAIN_CHANNEL_SECONDS) {
                    cancel();
                    channelingEarth.remove(uuid);

                    // ── 50/50 Water+Earth rune cost, plus 1 Sand block ─────────
                    int haveWater = countRunesDual(player, MagicElement.WATER);
                    int haveEarth = countRunesDual(player, MagicElement.EARTH);
                    boolean haveSand = playerHasBlock(player, Material.SAND);
                    if (haveWater < SANDRAIN_WATER_RUNE_COST || haveEarth < SANDRAIN_EARTH_RUNE_COST || !haveSand) {
                        player.sendActionBar("§c✗ §7Sand Rain fizzled! §8Need §b" + SANDRAIN_WATER_RUNE_COST
                                + " Water§8/§2" + SANDRAIN_EARTH_RUNE_COST + " Earth §8Runes §7+ §eSand§7.");
                        player.sendMessage("§c✗ §7The Sand Rain channel requires §b" + SANDRAIN_WATER_RUNE_COST
                                + " Water Runes§7, §2" + SANDRAIN_EARTH_RUNE_COST + " Earth Runes§7, and §e1 Sand block§7.");
                        player.playSound(player.getLocation(), Sound.ITEM_BUCKET_EMPTY, 0.8f, 0.6f);
                        return;
                    }
                    for (int i = 0; i < SANDRAIN_WATER_RUNE_COST; i++) consumeRuneDual(player, MagicElement.WATER);
                    for (int i = 0; i < SANDRAIN_EARTH_RUNE_COST; i++) consumeRuneDual(player, MagicElement.EARTH);
                    removeOneFromInventory(player, Material.SAND);

                    // 20% chance to shatter half remaining Earth runes — happens
                    // once the ritual COMPLETES (the "strain of the ritual"),
                    // matching Water's pattern of rolling on ritual attempt but
                    // moved here per design intent for consistency at completion.
                    rollRuneBreak(player, MagicElement.EARTH);

                    boolean activated = weatherStormManager.activateSandRain(player);
                    if (!activated) {
                        player.sendActionBar("§c✗ §7Sand Rain needs an active §bDownpour§7!");
                        return;
                    }
                    awardMagicXp(player, MAGIC_XP_CAST);
                    player.sendActionBar("§2☠ §e§lSAND RAIN! §7The rain turns to stinging sand!");
                    player.sendMessage("§2☠ §e§lSAND RAIN! §7The ritual is complete!");
                    return;
                }

                player.getWorld().playSound(player.getLocation(), Sound.BLOCK_SAND_STEP, 0.6f, 1.0f);
                player.getWorld().spawnParticle(Particle.BLOCK, player.getLocation().add(0, 1, 0), 15, 0.5, 0.5, 0.5, 0.0,
                        Material.SAND.createBlockData());
                player.getWorld().spawnParticle(Particle.GUST, player.getLocation().add(0, 1, 0), 4, 0.5, 0.3, 0.5, 0.02);

                player.sendActionBar("§2☠ §7Channelling Sand Rain... §e"
                        + (EARTH_SANDRAIN_CHANNEL_SECONDS - seconds) + "s §2☠");
            }
        }.runTaskTimer(plugin, 0L, 20L);
    }
    ...
    private void placeWaterStream(Player player, Block hitBlock) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if (b.getType().isAir() || b.getType() == Material.WATER) {
                ...
                // Tag as "magic water" — required for the Earth Staff's Sandstorm gate.
                Location wLoc = b.getLocation().toBlockLocation();
                magicWaterBlocks.add(wLoc);
                // Auto-expire the magic-water tag after 60s so old puddles don't
                // permanently count (the physical water block itself may persist
                // via vanilla water physics, but it stops counting as "magic").
                plugin.getServer().getScheduler().runTaskLater(plugin,
                    () -> magicWaterBlocks.remove(wLoc), 1200L);
            }
            else break;
        }
    }
    ...
    public void onProjectileHit(ProjectileHitEvent event) {
        ...
        int          lvl       = projectileLevels.getOrDefault(projId, 1);
        projectileLevels.remove(projId);
        // Fetch (and remove) the EarthBlockTier BEFORE any early-return so that
        // block-hit handling below can use the actual thrown tier material —
        // previously this was fetched only AFTER the hit-entity check, meaning
        // any block hit (wall/floor — the overwhelmingly common case for a
        // right-click block-throw) never knew which tier was thrown and always
        // placed plain DIRT, making expensive tier throws look like they did
        // nothing when aimed at terrain instead of a mob.
        ...
        if (event.getHitBlock() != null) {
            handleBlockHit(event.getHitBlock(), event.getHitBlockFace(), element, shooter, lvl, earthTier);
        }
    ...
    private void handleBlockHit(Block block, BlockFace face, MagicElement element,
                                 Player shooter, int lvl, EarthBlockTier tier) {
        Material type = block.getType();
        // The block material to place for Earth-staff throws — uses the actual
        // thrown tier's block (e.g. OBSIDIAN, IRON_BLOCK) instead of always
        // plain DIRT. This was the root cause of "Earth Staff right-click
        // block-throw wastes a rune and does nothing": aiming at a wall/floor
        // (the overwhelmingly common case) silently placed low-tier dirt
        // regardless of which expensive block was actually thrown.
        Material placeMat = (tier != null) ? tier.material : Material.DIRT;
        ...
        if (element == MagicElement.FIRE) {
            ...
            if (type == Material.LAVA) {
                ...
                // Earth + solid block (wall/floor) → place the ACTUAL thrown
                // tier's block on the face (e.g. OBSIDIAN, IRON_BLOCK), not
                // always plain DIRT. This is the core fix for "Earth Staff
                // right-click block-throw wastes a rune and does nothing" —
                // previously any wall/floor hit silently placed cheap dirt
                // regardless of the expensive block that was thrown, making
                // it look like nothing happened.
                Block adjacent = block.getRelative(face);
                if (adjacent.getType().isAir()) {
                    adjacent.setType(placeMat);
                    ...
                    adjacent.getWorld().spawnParticle(Particle.BLOCK,
                        ...
                        placeMat.createBlockData());
                    adjacent.getWorld().playSound(adjacent.getLocation(), Sound.BLOCK_GRAVEL_PLACE, 1.0f, 0.9f);
                    if (shooter != null) {
                        shooter.sendActionBar("§2§7Placed " + (tier != null ? tier.displayName : "§7Dirt") + "§7!");
                    }
                    final Block b = adjacent;
                    final Material placedMat = placeMat;
                    // Only basic dirt bolts (no tier) auto-clear after 30s.
                    // Actual thrown tier blocks are PERMANENT (matches the
                    // "hit an entity" placement behavior) so expensive throws
                    // aren't wasted — Silk Touch can recover them.
                    if (tier == null) {
                        plugin.getServer().getScheduler().runTaskLater(plugin, () -> {
                            if (b.getType() == placedMat) b.setType(Material.AIR);
                            magicDirtBlocks.remove(b.getLocation().toBlockLocation());
                        }, 600L);
                    }
                }
    ...
    private void handleFireHit(LivingEntity target, Player shooter, int lvl) {
        ...
        // WET → extinguishes any active fire ticks on impact (simple interaction, not a combo).
        if (isWet(target)) {
        ...
        // Normal fire hit — plain damage, no combo status chain.
        double dmg = 2.0 + SkillBonusManager.magicDamageBonus(lvl) * 2;
        target.damage(dmg, shooter);
        if (shooter != null) {
            shooter.sendActionBar("§c§7Fire blast hit!");
        }
        // ── Passive Fire Proc — real dice roll, gated by page + level ────────
        // Applies BURN (fire DoT + slow). Target can channel Downpour to
        // extinguish it early (see startWaterDownpourChannel).
        if (elementalProcManager != null && shooter != null) {
            elementalProcManager.rollAndApply(shooter, target, MagicElement.FIRE);
        }
    ...
    private void handleWaterHit(LivingEntity target, Player shooter, int lvl) {
        ...
        // Normal water hit — plain damage, no combo status chain.
        double dmg = 2.0 + SkillBonusManager.magicDamageBonus(lvl) * 2;
        target.damage(dmg, shooter);
        target.getWorld().spawnParticle(Particle.SPLASH,
            target.getLocation().add(0, 1, 0), 40, 0.4, 0.4, 0.4, 0.2);
        if (shooter != null) {
            shooter.sendActionBar("§b§7Water hit!");
        }
        // ── Passive Water Proc — real dice roll, gated by page + level ───────
        // Applies WET (slow debuff). Chance is boosted while an active
        // Downpour is raining in the target's world.
        if (elementalProcManager != null && shooter != null) {
            double downpourBonus = (weatherStormManager != null
                    && weatherStormManager.isWeatherEventActive(target.getWorld())
                    && !weatherStormManager.isSandRainActive(target.getWorld()))
                    ? ElementalProcManager.DOWNPOUR_WET_BONUS : 0.0;
            elementalProcManager.rollAndApply(shooter, target, MagicElement.WATER, downpourBonus);
        }
        rollMindBomb(target, shooter);
    }
    ...
    private void handleEarthHit(LivingEntity target, Player shooter, int lvl, EarthBlockTier tier) {
        ...
        // ── TIER SYSTEM (Level 10+): places the thrown block, deals damage ────
        // No trap/suffocate 2-hit chain anymore — every hit is a single,
        // independent event. Blocks are PERMANENT (Silk Touch recoverable).
        if (tier != null) {
            ...
            double dmg = tier.trapDamage + SkillBonusManager.magicDamageBonus(lvl) * 2;
            target.damage(dmg, shooter);
            target.addPotionEffect(new PotionEffect(PotionEffectType.SLOWNESS, 60, 1, false, true, true));

            int hearts = (int) (dmg / 2);
            if (shooter != null)
                shooter.sendActionBar("§2§7" + tier.displayName + " §7hit them! §8(§c-" + hearts + "❤§8)");
            if (target instanceof Player tp)
                tp.sendActionBar("§2§7Struck by " + tier.displayName + "§7!");
            if (shooter != null) awardMagicXp(shooter, MAGIC_XP_HIT);
        } else {
            // ── Basic bolt (no tier — Lv<10 or no matching block/page): plain damage ──
            double dmg = 2.0 + SkillBonusManager.magicDamageBonus(lvl) * 2;
            target.damage(dmg, shooter);
        ...
        // ── Passive Earth Proc — real dice roll, gated by page + level ───────
        // Applies MUDDY (heavy slow). Escaped early by mining a block with a
        // pickaxe (see onMuddyPickaxeBreak below).
        if (elementalProcManager != null && shooter != null) {
            elementalProcManager.rollAndApply(shooter, target, MagicElement.EARTH);
        }
        rollMindBomb(target, shooter);
    }

    /**
     * Mining a block with a pickaxe while Muddy breaks the debuff early —
     * the escape mechanic for the Earth proc.
     */
    @EventHandler(priority = EventPriority.MONITOR, ignoreCancelled = true)
    public void onMuddyPickaxeBreak(org.bukkit.event.block.BlockBreakEvent event) {
        Player player = event.getPlayer();
        if (elementalProcManager == null || !elementalProcManager.isMuddyProc(player)) return;
        ItemStack hand = player.getInventory().getItemInMainHand();
        if (hand == null || !hand.getType().name().endsWith("_PICKAXE")) return;
        elementalProcManager.clearMuddy(player);
    }
    ...
    private void handleAirHit(LivingEntity target, Player shooter, int lvl) {
        // WET → gust knocks it off (interaction, not a combo chain) — kept simple.
        if (target instanceof Player tp) {
        ...
        // Normal air gust — plain knockback + damage, no combo status chain.
        ...
        if (target instanceof Player tp2)
            tp2.sendActionBar(String.format("§7[Air] §f§lBlasted ~%.0f blocks!", kb));

        // ── Passive Air Proc — real dice roll, gated by page + level ─────────
        // Applies Chilled/Frozen (stun scaling with caster's Magic level).
        // On monsters ONLY, also rolls a rare flat instant-kill chance.
        if (elementalProcManager != null && shooter != null) {
            elementalProcManager.rollAndApply(shooter, target, MagicElement.AIR);
        }
        rollMindBomb(target, shooter);
    }
    ...
    /**
     * Getting hit while channeling the Water Downpour ritual cancels it
     * immediately — channeling requires full concentration.
     */
    @EventHandler(priority = EventPriority.MONITOR)
    public void onChannelHitCancel(org.bukkit.event.entity.EntityDamageEvent event) {
        if (!(event.getEntity() instanceof Player p)) return;
        if (event.isCancelled()) return;
        UUID uid = p.getUniqueId();
        if (channelingWater.remove(uid)) {
            p.sendActionBar("§c✗ §7Downpour Spell cancelled — you were hit!");
            p.playSound(p.getLocation(), Sound.ITEM_BUCKET_EMPTY, 0.8f, 0.6f);
        }
        if (channelingEarth.remove(uid)) {
            p.sendActionBar("§c✗ §7Sand Rain channel cancelled — you were hit!");
            p.playSound(p.getLocation(), Sound.ITEM_BUCKET_EMPTY, 0.8f, 0.6f);
        }
    }
    ...
    /** PDC/metadata key for the "Damp" companion status (slowness that rides along with WET). */
    public static final String META_DAMP = "magic_damp";
    ...
    public void applyWet(LivingEntity e, int ticks) {
        ...
        // ── Damp companion status ─────────────────────────────────────────────
        // Being Wet also makes the target Damp — a mild slow that rides along
        // with WET and is the gate for the lightning-vs-wet damage bonus below.
        e.setMetadata(META_DAMP, new FixedMetadataValue(plugin,
            System.currentTimeMillis() + (long) ticks * 50));
        e.addPotionEffect(new PotionEffect(PotionEffectType.SLOWNESS, ticks, 0, false, true, true));
        if (e instanceof Player p) {
            p.sendActionBar("§b§7You are §bDamp§7! §8(Slowed - lightning deals bonus damage while Wet/Damp)");
        }
    }

    /** Returns true if the entity currently has the Damp status (rides with WET). */
    public boolean isDamp(LivingEntity e) {
        if (!e.hasMetadata(META_DAMP)) return false;
        long ex = (long) e.getMetadata(META_DAMP).get(0).value();
        if (System.currentTimeMillis() > ex) { e.removeMetadata(META_DAMP, plugin); return false; }
        return true;
    }
    ...
    /**
     * Counts {@code element} runes across BOTH the player's main inventory
     * AND their Magic Bag (if wired in). Used by systems that must treat the
     * bag as an extended inventory (e.g. Downpour upkeep, Sand Rain channel
     * cost) so players aren't falsely told they're out of runes when they
     * actually have them stashed in the bag.
     */
    private int countRunesDual(Player player, MagicElement element) {
        int total = countRunes(player, element);
        ItemStack[] bag = getBagContents(player);
        if (bag != null) {
            for (ItemStack s : bag) {
                if (s != null && itemFactory.isRune(s, element)) total += s.getAmount();
            }
        }
        return total;
    }
    ...
    /**
     * Consumes 1 {@code element} rune, checking the player's main inventory
     * FIRST, then falling back to their Magic Bag if none found there.
     * Returns false if no rune was found in either location.
     */
    private boolean consumeRuneDual(Player player, MagicElement element) {
        if (consumeRune(player, element)) return true;

        ItemStack[] bag = getBagContents(player);
        if (bag != null && magicBagManager != null) {
            for (int i = 0; i < bag.length; i++) {
                ItemStack s = bag[i];
                if (s == null || !itemFactory.isRune(s, element)) continue;
    ...
    /**
     * 20% chance to break HALF of the player's {@code element} runes (in main
     * inventory or Magic Bag) when right-click casting Earth (Sand Rain) or
     * Water (Downpour). Rolled once per successful right-click cast attempt.
     */
    private void rollRuneBreak(Player player, MagicElement element) {
        if (RAND.nextInt(100) >= 20) return; // 20% chance

        int total = countRunes(player, element);
        // Also count runes stored in the Magic Bag
        ItemStack[] bag = getBagContents(player);
        if (bag != null) {
            for (ItemStack s : bag) {
                if (s != null && itemFactory.isRune(s, element)) total += s.getAmount();
        ...
        int toBreak = Math.max(1, total / 2);
        int broken = 0;

        // Break from main inventory first
        ItemStack[] contents = player.getInventory().getContents();
        for (int i = 0; i < contents.length && broken < toBreak; i++) {
            ItemStack item = contents[i];
            if (item == null || !itemFactory.isRune(item, element)) continue;
            int take = Math.min(item.getAmount(), toBreak - broken);
            item.setAmount(item.getAmount() - take);
            if (item.getAmount() <= 0) player.getInventory().setItem(i, new ItemStack(Material.AIR));
            broken += take;
        }

        // Then break from Magic Bag if still needed
        if (broken < toBreak && bag != null && magicBagManager != null) {
            for (int i = 0; i < bag.length && broken < toBreak; i++) {
                ItemStack s = bag[i];
                if (s == null || !itemFactory.isRune(s, element)) continue;
                int take = Math.min(s.getAmount(), toBreak - broken);
                s.setAmount(s.getAmount() - take);
                if (s.getAmount() <= 0) bag[i] = null;
                broken += take;
            }
            magicBagManager.saveAsync(player.getUniqueId());
        }
        ...
        if (broken > 0) {
            player.getWorld().spawnParticle(Particle.ITEM_SNOWBALL,
                player.getLocation().add(0, 1, 0), 20, 0.4, 0.4, 0.4, 0.05);
            player.playSound(player.getLocation(), Sound.ITEM_SHIELD_BREAK, 1.0f, 0.7f);
            player.sendMessage("§c✗ §7The strain of the ritual shattered §e" + broken
                    + " §7of your " + element.color + element.name() + " Runes§7!");
            player.sendActionBar("§c✗ §7" + broken + " " + element.color + element.name() + " Runes §7broke!");
        }
    }
    ...
    private void activateAirHover(Player player, int magicLevel) {
        ...
        if (magicLevel < 99) {
            ...
            // Rune cost scales with level: 1 rune per click, +1 rune per 8 levels
            int runeCost = 1 + (magicLevel / 8);
            int haveRunes = countRunes(player, MagicElement.AIR);
            if (haveRunes < runeCost) {
                player.sendActionBar("§c✗ §7Need §f" + runeCost + " §7Air Runes for Air Boost §8(have " + haveRunes + ")§7!");
                return;
            }
            for (int i = 0; i < runeCost; i++) {
                if (!consumeRune(player, MagicElement.AIR)) {
                    // Shouldn't happen since we pre-checked count, but bail safely if it does.
                    player.sendActionBar("§c✗ §7Ran out of §fAir Runes§7 mid-cast!");
                    return;
                }
            }
        ...
        // Level 99+: Continuous Levitation/Flight — flat 3 Air Runes per second
        if (permaFlightActive.contains(uid)) {
            cancelPermaFlight(uid);
        ...
        // Need at least 3 runes to start (1 second's worth)
        if (countRunes(player, MagicElement.AIR) < 3) {
            player.sendActionBar("§c✗ §7Need §f3 §7Air Runes to start flying §8(3/sec drain)§7!");
            return;
        }
        for (int i = 0; i < 3; i++) consumeRune(player, MagicElement.AIR);

        permaFlightActive.add(uid);

        // Apply initial effects
        player.addPotionEffect(new PotionEffect(PotionEffectType.SLOW_FALLING, 30, 0, false, true, true));
        player.addPotionEffect(new PotionEffect(PotionEffectType.LEVITATION, 30, 0, false, false, false));
        ...
        player.sendActionBar("§7☁ §f⬆ FLYING §8— Drains §f3 §7Air Runes/sec. Right-click to stop.");

        // Sustain task: runs every 1 second (20 ticks), consumes 3 runes each tick
        BukkitTask task = plugin.getServer().getScheduler().runTaskTimer(plugin, () -> {
            if (!player.isOnline()) { cancelPermaFlight(uid); return; }
            if (player.isOnGround()) {
                cancelPermaFlight(uid);
            ...
            // Consume 3 Air Runes per second — abort immediately if unable to satisfy all 3
            if (countRunes(player, MagicElement.AIR) < 3) {
                cancelPermaFlight(uid);
                player.removePotionEffect(PotionEffectType.LEVITATION);
            ...
            player.sendActionBar("§7☁ §f⬆ Flying §8— §f" + remaining + " §7Air Rune" + (remaining == 1 ? "" : "s") + " left §8(-3/sec)");
        }, 20L, 20L);
        permaFlightTasks.put(uid, task);
    }

    /** Cancels the Lv99 permanent flight task and clears active-flight state for the given player. */
    private void cancelPermaFlight(UUID uid) {
        permaFlightActive.remove(uid);
        BukkitTask t = permaFlightTasks.remove(uid);
        if (t != null) t.cancel();
    }
