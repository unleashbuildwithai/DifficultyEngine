...
public class CastingEngine implements Listener {
    ...
    /** Optional MagicStaffListener — wired in so Support Staff left-click can fire real per-element spells. */
    private com.yourname.difficulty.magic.MagicStaffListener magicStaffListener = null;
    /** Optional SkillManager — wired in so real spell casts can scale with the player's actual Magic level. */
    private com.yourname.difficulty.skills.SkillManager skillManager = null;
    ...
    public CastingEngine(JavaPlugin plugin,
    ...
    /** Wires in the MagicStaffListener so Support Staff left-click can fire real per-element spells. */
    public void setMagicStaffListener(com.yourname.difficulty.magic.MagicStaffListener msl) { this.magicStaffListener = msl; }
    /** Wires in the SkillManager so real spell casts scale with the player's actual Magic level. */
    public void setSkillManager(com.yourname.difficulty.skills.SkillManager sm) { this.skillManager = sm; }


    // ── Armed Support Effect (Blessing Potion arming/discharge) ──────────────
    ...
    private void executeSupportBasicAttack(Player player) {
        ...
        // ── Fire each active element's REAL basic-tier spell ──────────────────
        // Per spec: instead of a simplified combined snowball, dispatch to
        // MagicStaffListener.castElementForSupport() for each active element so
        // the actual elemental spell (fireball, water bolt, earth bolt, air
        // gust) is cast. Runes for each element were already consumed above.
        StringBuilder sb = new StringBuilder("§5✦ §7Support Flurry: ");
        if (magicStaffListener != null) {
            int lvl = skillManager != null
                    ? skillManager.getLevel(player.getUniqueId(), com.yourname.difficulty.skills.SkillType.MAGIC)
                    : 1;
            if (fireActive)  { sb.append("§cFire "); magicStaffListener.castElementForSupport(player, MagicElement.FIRE, lvl); }
            if (waterActive) { sb.append("§bWater "); magicStaffListener.castElementForSupport(player, MagicElement.WATER, lvl); }
            if (earthActive) { sb.append("§2Earth "); magicStaffListener.castElementForSupport(player, MagicElement.EARTH, lvl); }
            if (airActive)   { sb.append("§fAir "); magicStaffListener.castElementForSupport(player, MagicElement.AIR, lvl); }
            sb.append("§8(real spells fired!)");
        } else {

            // Fallback (MagicStaffListener not wired) — old simplified combined snowball
            ...
            new org.bukkit.scheduler.BukkitRunnable() {
                ...
                public void run() {
                    ...
                    ticksLived++;
                    if (ticksLived < 3) return;

                    Location loc = snowball.getLocation();
                    loc.getWorld().spawnParticle(Particle.ELECTRIC_SPARK, loc, 1, 0.01, 0.01, 0.01, 0.0);
                    if (fireActive)  loc.getWorld().spawnParticle(Particle.FLAME, loc, 1, 0.01, 0.01, 0.01, 0.01);
                    if (waterActive) loc.getWorld().spawnParticle(Particle.SPLASH, loc, 1, 0.01, 0.01, 0.01, 0.01);
                    if (earthActive) loc.getWorld().spawnParticle(Particle.HAPPY_VILLAGER, loc, 1, 0.01, 0.01, 0.01, 0.01);
                    if (airActive)   loc.getWorld().spawnParticle(Particle.CLOUD, loc, 1, 0.01, 0.01, 0.01, 0.01);
                }
            }.runTaskTimer(plugin, 1L, 1L);
        }
